...
def main():
    ...
    if answer.upper() == "Y":
        custom_answer = input("Is there a previously defined marker file you want to use? Y/N")

        if custom_answer.upper() == "Y":
            json_marker_filename = input("Input name of previously defined marker JSON file.")

            while not os.path.isfile(json_marker_filename):
                json_marker_filename = input("Error; invalid file. Input name of previously defined marker JSON file.")
